			...
			if(Vdist(pos.x, pos.y, pos.z, j.positionFrom.x, j.positionFrom.y, j.positionFrom.z) < 7.0)then
				...
				if(Vdist(pos.x, pos.y, pos.z, j.positionFrom.x, j.positionFrom.y, j.positionFrom.z) < 5.0)then
					...
					elseif propietario == 1 then
						...
						--Drawing.draw3DText(j.positionFrom.x, j.positionFrom.y, j.positionFrom.z - 1.500, "Porta: "..puertaesta, 4, 0.2, 0.1, 255, 255, 255, 215)
						--[[if soyelpropietario == 1 then
							if estado == 0 then 
								Drawing.draw3DText(j.positionFrom.x, j.positionFrom.y, j.positionFrom.z - 1.800, "Pressionar ~b~G~w~ para ~g~Abrir~w~ ~w~a porta", 4, 0.1, 0.1, 255, 255, 255, 215)
							else 
								Drawing.draw3DText(j.positionFrom.x, j.positionFrom.y, j.positionFrom.z - 1.800, "Pressionar ~b~G~w~ para ~r~Fechar ~w~a porta", 4, 0.1, 0.1, 255, 255, 255, 215)
							end
						end]]
					end
			...
			if(Vdist(pos.x, pos.y, pos.z, j.positionTo.x, j.positionTo.y, j.positionTo.z) < 7.0)then
				...
				if(Vdist(pos.x, pos.y, pos.z, j.positionTo.x, j.positionTo.y, j.positionTo.z) < 5.0)then
					...
					elseif propietario == 1 then
						...
						--Drawing.draw3DText(j.positionTo.x, j.positionTo.y, j.positionTo.z - 1.300, "Porta: "..puertaesta, 4, 0.2, 0.1, 255, 255, 255, 215)
						--[[if soyelpropietario == 1 then
							Drawing.draw3DText(j.positionTo.x, j.positionTo.y, j.positionTo.z - 1.600, "Presiona ~b~G~w~ para ~g~Abrir~w~/~r~Fechar ~w~a porta", 4, 0.05, 0.05, 255, 255, 255, 215)
						end]]
					end
...
function AbrirMenuArmario(propiedad)
 ...
 ESX.TriggerServerCallback('pk_casas:getInventoryV', function(inventory)
	...
	--if mdinero == 1 then
		table.insert(elements, {label = 'Dinheiro Sujo €' .. inventory.blackMoney, type = 'item_account', value = 'black_money'})
	--end
          ...
          ESX.UI.Menu.Open(
            ...
            function(data2, menu)
              ...
              if quantity == nil or quantity < 1 then
                --ESX.ShowNotification('Quantidade Inválida')
				exports['mythic_notify']:SendAlert('error', 'Quantidade Inválida!')
			  else
...
function AbrirMenuMejoras(casanombre,preciogarage,precioarmas,preciodinero,precioitems)
	...
	if mgarage > 0 and preciogarage > 0 then
		table.insert(elements, {label = 'Melhora de garagem: ADQUIRIDA', value = 'no'})
	elseif mgarage == 0 and  preciogarage > 0 then
		table.insert(elements, {label = 'Construir garagem por €'..preciogarage, value = 'mejorasgarage'})
	elseif preciogarage == 0 then
		table.insert(elements, {label = 'Melhora de garagem: Não disponível', value = 'no'})
	end
	
	if marmas == 0 and precioarmas > 0 then
		table.insert(elements, {label = 'Construir cofre para armas €'..precioarmas, value = 'mejorasarmas'})
	elseif marmas > 0 and precioarmas > 0 then 
		table.insert(elements, {label = 'Melhora almacen de armas: ADQUIRIDA', value = 'no'})
	elseif precioarmas == 0 then 
		table.insert(elements, {label = 'Melhora almacen de armas: NO DISPONIBLE', value = 'no'})
	end
	...
	elseif mdinero > 0 and preciodinero > 0 then 
		table.insert(elements, {label = 'Melhora almacen de dinero: ADQUIRIDA', value = 'no'})
	elseif preciodinero == 0 then
		table.insert(elements, {label = 'Melhora almacen de dinero: NO DISPONIBLE', value = 'no'})
	end
	...
	elseif mitems > 0 and precioitems > 0 then 
		table.insert(elements, {label = 'Melhora almacen de items: ADQUIRIDA', value = 'no'})
	elseif precioitems == 0 then
		table.insert(elements, {label = 'Melhora almacen de items: NO DISPONIBLE', value = 'no'})
	end
...
function AbrirMenuNoPropietario(casanombre,precio, cordx, cordy, cordz, entrarosalir)
	...
	local pos = GetEntityCoords(GetPlayerPed(-1), true)
	local elements = {
		{label = "💲 Comprar propiedade" ,value = "comprar"},
	...
	function(data, menu)
		...
		if data.current.value == "cerrar" then 
			...
			if estado == 1 then
				...
				TriggerServerEvent('pk_casas:checkcasa',casanombre)
			elseif estado == 0 then
				--TriggerEvent('chatMessage', 'PROPIEDAD', {255, 0, 0},"La casa ya se encuentra Fechada.")
	...
	while true do
		...
		if mgarage == 1 then
			if(Vdist(pos.x, pos.y, pos.z, j.GuardarVehiculo.x, j.GuardarVehiculo.y, j.GuardarVehiculo.z) < 6.0)then
				...
				if(Vdist(pos.x, pos.y, pos.z, j.GuardarVehiculo.x, j.GuardarVehiculo.y, j.GuardarVehiculo.z) < 6.0)then
				Drawing.draw3DText(j.GuardarVehiculo.x, j.GuardarVehiculo.y, j.GuardarVehiculo.z - 1.100, "E - Guardar Veículo", 4, 0.2, 0.1, 255, 255, 255, 215)
					if(Vdist(pos.x, pos.y, pos.z, j.GuardarVehiculo.x, j.GuardarVehiculo.y, j.GuardarVehiculo.z) < 2.0)then
						ClearPrints()
						SetTextEntry_2("STRING")
						--AddTextComponentString("Pressionar ~r~E~w~ para guardar")--
						DrawSubtitleTimed(1000, 1)
						if IsControlJustPressed(1, 38) then
							TriggerServerEvent('pk_casas:checkcasa',j.GuardarVehiculo.nom)
							if soyelpropietario == 1 then
		...
		if mgarage == 2 then
			...
			if IsInVehicle() then
				if(Vdist(pos.x, pos.y, pos.z, j.GuardarVehiculo.x, j.GuardarVehiculo.y, j.GuardarVehiculo.z) < 6.0)then
				Drawing.draw3DText(j.GuardarVehiculo.x, j.GuardarVehiculo.y, j.GuardarVehiculo.z - 1.100, "E - Retirar Veículo", 4, 0.2, 0.1, 255, 255, 255, 215)
					if(Vdist(pos.x, pos.y, pos.z, j.GuardarVehiculo.x, j.GuardarVehiculo.y, j.GuardarVehiculo.z) < 2.0)then
						ClearPrints()
						SetTextEntry_2("STRING")
						--AddTextComponentString("Pressionar ~r~E~w~ para retirar veículo")--
						DrawSubtitleTimed(1000, 1)
						if IsControlJustPressed(1, 38) then
							TriggerServerEvent('pk_casas:checkcasa',j.GuardarVehiculo.nom)
							if soyelpropietario == 1 then
...
function ranger(vehicle,vehicleProps)
	...
	exports['mythic_notify']:SendAlert('success', 'Veículo guardado!')
end
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		ESX.TriggerServerCallback('pk_garage:stockv',function(valid)
			...
			if (valid) then			      
					...
					exports['mythic_notify']:SendAlert('error', 'Este veículo não te pertence!')
			end
		end,vehicleProps)
	else		
		--ESX.ShowNotification('Notificación de garaje: Tu vehículo debe estar en el marcador')
		exports['mythic_notify']:SendAlert('error', 'Não se encontra no marcador!')
	end
...
function reparation(vehicle,vehicleProps,propiedad)
	...
	local elements = {
		{label = "🚗 Guardar veículo", value = 'yes'},
		{label = "❌ Cancelar", value = 'no'},
	}
	ESX.UI.Menu.Open(
